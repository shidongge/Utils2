...
public class SwipeRefreshView extends SwipeRefreshLayout {
    ...
    private boolean canLoadMore() {
        ...
        if (mGv != null && mGv.getAdapter() != null) {
            condition2 = mGv.getLastVisiblePosition() == (mGv.getAdapter().getCount() - 1);
        }
    ...
    public void setLoading(boolean loading) {
        ...
        if (isLoading) {
            // 显示布局
            mGv.addView(mFooterView);
        } else {
            // 隐藏布局
            mGv.removeView(mFooterView);
            // 重置滑动的坐标
            mDownY = 0;
            mUpY = 0;
        }
